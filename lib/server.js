...
function run() {
    ...
    app.get('*', function (req, res, next) {
        if(req.params[0] == mainPath) {
            ...
            var js, error;
            try {
                js = concatenator.concat(config.dependencies, srcDir); //for now only js files are supported
    ...
    plugins.each(function (pluginMod) {
        ...
        app.get(matchRequest, function (req, res, next) {
            ...
            if(req.params[0] == mainPath)
                return processAsMain(inputFile, res);
            ...
            try {
                ...
                pluginMod.server(body, function (e, body) {
                    if(e)
                        throw e;
                    res.send(body);
                });
