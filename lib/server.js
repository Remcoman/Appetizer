...
function run() {
    ...
    if(config.proxies) {
        Object.keys(config.proxies).forEach(function (proxy) {
            var redirect = config.proxies[proxy].replace(/\/$/, "");

            var regex = new RegExp("^/" + proxy + "(/.*)?");
            app.get(regex, function (req, res, params) {
                http.get(redirect + (req.params[0] || ""), function (remoteRes) {
