...
function copyPaths(next) {
    ...
    //put all the extensions in a hashmap for easy lookup
    var toExcludeMap = (function () {
        var map = {};
        toExclude.forEach(function (pattern) {
            map[pattern.replace(/^\*\./, "").toLowerCase()] = true;
        });
        return map;
    })();
    ...
    var copyPath = function (from, to, callback) {
        ...
        if(stat.isFile()) {
            var extension = path.extname(from);

            if(toExcludeMap.hasOwnProperty(extension))
...
function processMain(next) {
    ...
    process.nextTick(function () {
        ...
        if(mode == "release")
            js = packer.pack(js, false, config.obfuscate_releasebuild);
