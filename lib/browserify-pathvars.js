 ...
 * @return {Function}
 */
module.exports = function (locales, userVars, extraVars) {
    ...
    var getVars = function (file) {
        var vars = [], match, regex = /[\\]?\$\{([^}]+)\}/g;
        ...
        //for each path part check if its a var. If so then replace it.
        file.split(path.sep).forEach(function (part) {
            if(part.charAt(0) == "$")
                vars.push( part.substr(1) );
        });

        return vars;
    }

    var hasVar = function (file, varName) {
        return getVars(file).indexOf(varName) > -1;
    }
